...
struct PQIndex {
    ...
    search(const float* query, size_t k) const {
        ...
        std::vector<std::pair<float,uint32_t> > all_dis(base_number);
        ...
        for (; i + 3 < base_number; i += 4) {
            ...
            float dis[4];
            vst1q_f32(dis, dis_vec);
            all_dis[i+0] = std::make_pair(dis[0], (uint32_t)(i+0));
            all_dis[i+1] = std::make_pair(dis[1], (uint32_t)(i+1));
            all_dis[i+2] = std::make_pair(dis[2], (uint32_t)(i+2));
            all_dis[i+3] = std::make_pair(dis[3], (uint32_t)(i+3));
        }
        for (; i < base_number; ++i) {
            float ip = 0;
            for (size_t m = 0; m < M; ++m)
                ip += lut.data()[m*Ks + c[i*M+m]];
            all_dis[i] = std::make_pair(1.f - ip, (uint32_t)i);
        }

        std::partial_sort(all_dis.begin(),
                          all_dis.begin() + k,
                          all_dis.end());
